...
直接拿 LLM 問投資問題,有四個結構性缺陷;本專案的每一個設計都是針對其中一個而來:

| 問題 | 直接問 LLM 會怎樣 | 本專案的解法 |
|------|------|------|
| **幻覺與心算錯誤** | 目測 K 線、心算 RSI/MACD,數字看似合理實則捏造;訓練資料截止後的行情一概不知 | 歷史 K 線存本地 SQLite,技術指標**只能**由 `compute_ta.py` 純 Python 確定性計算,LLM 只讀計算結果;現價與新聞永遠現抓並標註時間戳 |
| **資訊過時與單邊敘事** | 檢索到過時資料照樣引用(「有憑有據的錯誤」);搜尋結果被利多新聞洗版就跟著喊多 | Freshness Gate 給每類資料明確 TTL,過期即現抓;**強制反向查詢**(每標的至少 1 次搜尋找利空)+ 來源分層(官方一手 > 主流媒體 > 自媒體)+ 關鍵數字雙來源交叉驗證 |
| **確認偏誤與過度自信** | 順著提問者的立場給答案,永遠一副很有把握的樣子 | 評分前強制**多空對辯**(聰明的空頭會說什麼 ×3,禁止立稻草人);信心指數用寫死的規則扣分——訊號矛盾、資料稀缺、單一來源都必須誠實降信心,上限 90 |
| **說過就忘、無法問責** | 上週喊買、這週喊賣,沒人記得;預測對錯永遠無從檢驗 | 每筆判斷寫入 **append-only 預測日誌**,到期後用實際價格路徑判定 WIN/LOSS,統計勝率、期望值與對 benchmark 的超額報酬;觀點翻轉必須指出新事實,否則扣信心分 |

一句話總結:**把「AI 隨口給的投資意見」變成「流程固定、證據留痕、事後可對答案的決策系統」**——
系統好不好,不由它自己說,由累積的統計數據說。

同時也誠實承認邊界:分數是公開資訊的結構化綜合,不是 alpha 保證;LLM 不具真正
預測能力,這正是驗證框架存在的理由。所有輸出皆非投資建議。
