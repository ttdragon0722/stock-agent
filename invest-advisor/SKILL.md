...
description: >
  ...
  skill 會執行資訊蒐集(新聞、技術分析、基本面)並產出含 1-100 量化評分的
  Markdown 決策報告,同時自動記錄預測日誌供事後驗證。
---
...
`assets/report-template.md`。每份報告必含:四維度分數、多空對辯與裁決
(Step 3.5 產出)、失效條件(可證偽)、風險聲明、**參考資料來源清單
(附完整 URL 與查詢日期,規範見 report-templates.md「來源標註規範」)**、
`prediction-meta` 隱藏 JSON 區塊;有歷史紀錄時另含「與上次觀點的差異」
(Step 1.5 產出)。

**硬性關卡:缺「參考資料來源」一節的報告視為未完成——禁止存檔、
禁止進 Step 6 記錄預測。**同一對話的追問延伸報告(第 2、3 份)同樣適用。
