...
- **命名文法:`<YYYY-MM-DD>_<主題>_<類型>.md`**(日期在最前,底線分隔)
  ...
  - 類型:`Q1`–`Q6`(決策報告)或 `calibration`(校準報告)
  - 範例:`2026-07-04_TW-screener_Q3.md`、`2026-07-04_SOL-ONDO_Q1.md`、
    `2026-08-01_calibration.md`
  - Q6 日報:單標的 `2026-07-16_0050-TW_Q6.md`;總覽日報的彙總報告
    固定主題 `daily-summary`,如 `2026-07-16_daily-summary_Q6.md`
...
### 來源多樣性(cache_news --stats:獨立來源 N 個、flags、扣分)
...
**所有決策報告(Q1–Q6,含 Q6 總覽彙總)開頭必附「白話摘要」段落**,
位置在標題/產出時間之後、「一、結論摘要」之前。**缺此節的報告視為未完成**
——與「參考資料來源」「掛單表」同級的硬性關卡:禁止存檔、禁止記錄預測。
(calibration 校準報告由腳本產出,不在此列。)

寫法:

1. **3–6 句,寫給完全沒學過投資的初學者**:國中生能看懂的日常語言。
2. **禁用術語**;非用不可時括號一句白話解釋,如
   「支撐(很多人願意在這個價格買、跌到這裡常會止跌)」。
   推薦指數/信心指數可以出現,但要說明「滿分 100,越高越看好」。
3. **必答三件事**:
   - 這份報告在看什麼、為什麼現在看
   - 我們的判斷是什麼、最主要的一兩個理由
   - 建議你實際怎麼做(「先不要買」「等跌到 X 元再考慮買一點」
     「繼續放著,但跌破 X 元要賣」這種可直接照做的語言)
4. **有驗證前次建議的報告(如 Q6 日報)再答第四件事**:
   之前說的準不準、原本的做法要不要改。
5. **不得引入正文沒有的新資訊**,結論方向必須與 TL;DR 與裁決一致;
   風險提醒至少一句(如「這不是投資建議,錢是你的,最後自己決定」)。
...
6. **「來源多樣性」子節必附(rubric 1.3.0)**:寫出
   `cache_news.py --stats --symbol <symbol>` 的
   獨立來源數(去重網域)、觸發的 flags 與 `confidence_penalty`,
   格式如:「獨立來源 2 個(udn、cnyes);flags: weak_source_count →
   信心 −10 已計入」。只寫「命中 N 則」不算數——**則數 ≠ 來源數**。
7. **ETF 報告**:代理標的的新聞逐條標註「(代理:2330.TW)」,
   不得讓讀者誤以為是 ETF 本身的個股消息(見 data-sources.md §3.1)。
8. 同一則通稿的多家轉載只列**一條**(擇最原始者),其餘不重複佔位;
   跨標的共用的大盤新聞在每份報告都要列出,但獨立來源數各自計算。
...
**(a) 前次決策驗證表**(兩層驗證結果,見 question-types.md Q6):

```markdown
#### 前次決策驗證(正式判定依 verify_predictions.py,{驗證時間戳})

| 前次預測 (id) | 方向/關鍵點位 | 正式判定 | 現價對照 | 今日決策 |
|---------------|--------------|:-------:|----------|----------|
| 2026-07-15-0050.TW-q4 | long,回踩 102.8–104 | OPEN | 未觸發(現價 105.8,距上緣 +1.7%) | 掛單保留 |
| {id} | {…} | {OPEN/WIN/LOSS/FLAT/NOT_FILLED} | {已觸發/接近(≤1×ATR)/未觸發 + 具體價差} | {掛單保留/撤單/調整 至{價}/停止追蹤} |
```

規則:
1. 「正式判定」只能引 `verify_predictions.py --fetch` 的輸出,禁止心算
2. 「現價對照」逐條檢查前次掛單價、SL/TP、失效條件的觸發狀態,
   價差以現價與 compute_ta 的 ATR 為基準
3. **「今日決策」欄不得留空**;失效條件已觸發 → 必須「撤單」,
   禁止改寫成逢低加碼
4. 表格下方一句話總結:前次決策**是否仍有效、能否繼續執行**
5. 無前次紀錄 → 本段寫「首次分析,無前次決策可驗證」

**(b) 今日更新**:同 Q4 情境樹(突破/回踩/跌破,標注主情境),
外加「今日變化」小節:前收→現價的缺口與量能、籌碼增量(台股)、
當日新聞對前次論點的增強/削弱。

### Q6 總覽彙總(總覽日報時另存一份,主題 `daily-summary`)

```markdown
# 【日報總覽】{YYYY-MM-DD}
> 產出時間:{ISO timestamp}|關注標的:{n} 檔(來源:watchlist.py)

## 白話摘要   ← 必附,彙整版:今天總共看了幾檔、整體情況一句話、
##              哪幾檔要注意/要動作(白話),規範同「白話摘要規範」

| 標的 | 現價(日漲跌%) | 上次觀點 | 驗證結果 | 今日決策 | 推薦/信心 | 報告 |
|------|--------------|----------|----------|----------|----------|------|
| 0050.TW | 105.8(+0.6%) | 07-15 long 61 | OPEN,未觸發 | 掛單保留 | 61/55 | [連結](2026-07-16_0050-TW_Q6.md) |
```

規則:彙總只引各單標的日報的結論,不得出現單標的報告沒有的新判斷;
彙總報告**不記預測日誌、不跑事件萃取**(單標的報告已各自做完)。
...
Q2_long_hold|Q3_screener|Q3_screened_out|Q4_levels|Q5_allocation|Q6_daily),
...
`Q6_daily` 只在**觀點改變**時記錄(方向翻轉、關鍵點位調整 ≥ 1×ATR、
或前次預測已到期/已判定,見 question-types.md Q6);沿用前次觀點的日報
**不記新預測**,在報告中註明沿用的 prediction id 即可。
